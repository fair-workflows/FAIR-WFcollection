...
with open('annotation_steps_and_participants.json') as infile:
        ...
        if tags [0] == "step" :
            ...
            for entryInto in data:
                ...
                if startInto >= start and endInto <= end and tagsInto [0] != "step" :
                    assertion.add( (this_step, DCTERMS.hasPart, rdflib.term.URIRef(url + '#'+ tagsInto [0]) ))
                    assertion.add( (rdflib.term.URIRef(url + '#'+ tagsInto [0]), RDFS.label, rdflib.Literal(exactInto)) )
